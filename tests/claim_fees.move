...
module metrom::claim_fees_tests {
    ...
    #[test(
        aptos = @aptos_framework, metrom = @metrom, owner = @0x50
    )]
    ...
    fun success(
        ...
        tests_base::init(aptos);
        tests_base::init_metrom_with_defaults(metrom, owner);
        let (_, reward_token_address) = tests_base::create_default_rewards_campaign(owner, owner);

        let recipient = @0x70;
        assert!(test_token::balance_of(test_token::get_asset(reward_token_address), recipient) == 0);
        metrom::claim_fees(owner, reward_token_address, recipient);
        assert!(test_token::balance_of(test_token::get_asset(reward_token_address), recipient) == tests_base::octas(1));
    }
}
